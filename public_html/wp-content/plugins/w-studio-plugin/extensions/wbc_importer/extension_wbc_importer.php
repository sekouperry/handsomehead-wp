    ...
    class ReduxFramework_extension_wbc_importer {
                ...
                foreach ( $imports as $import ) {
                    ...
                    if ( $import['type'] == "d" && !empty( $import['name'] ) ) {
                        ...
                        foreach ( $import['files'] as $file ) {
                            switch ( $file['name'] ) {
                            case 'wstudiomain.xml':
                                $this->wbc_import_files['wbc-import-'.$x]['content_file'] = $file['name'];
                                break;
								
							case 'wstudiocreativeone.xml':
                                $this->wbc_import_files['wbc-import-'.$x]['content_file'] = $file['name'];
                                break;
							case 'wstudiofreelancer.xml':
                                $this->wbc_import_files['wbc-import-'.$x]['content_file'] = $file['name'];
                                break;
							case 'wstudiocreativemulti.xml':
                                $this->wbc_import_files['wbc-import-'.$x]['content_file'] = $file['name'];
                                break;
							case 'wstudiominimal.xml':
                                $this->wbc_import_files['wbc-import-'.$x]['content_file'] = $file['name'];
                                break;
							case 'wstudiophotographer.xml':
                                $this->wbc_import_files['wbc-import-'.$x]['content_file'] = $file['name'];
                                break;
							case 'wstudioartist.xml':
                                $this->wbc_import_files['wbc-import-'.$x]['content_file'] = $file['name'];
                                break;
							case 'wstudiophotographer2.xml':
                                $this->wbc_import_files['wbc-import-'.$x]['content_file'] = $file['name'];
                                break;
        ...
        function add_importer_section() {
            ...
            $this->parent->sections[] = array(
                ...
                'desc'   => '<p class="description">'. apply_filters( 'wbc_importer_description', __( 'Works best to import on a new install of WordPress. <br />Sometimes the demo content installer might cause your server to run out of memory during the installation. To combat this you can always up the memory limit that WordPress uses. <strong>Increase your memory limit before importing the demo content.</strong>', 'framework' ) ).'</p>',
