...
if ( ! class_exists( 'ReduxFramework_spacing' ) ) {
    class ReduxFramework_spacing {
        ...
        function render() {
            ...
            $value = array(
                'top'    => isset( $this->value[ $this->field['mode'] . '-top' ] ) ? $this->value[ $this->field['mode'] . '-top' ] : $this->value['top'],
                'right'  => isset( $this->value[ $this->field['mode'] . '-right' ] ) ? $this->value[ $this->field['mode'] . '-right' ] : $this->value['right'],
                'bottom' => isset( $this->value[ $this->field['mode'] . '-bottom' ] ) ? $this->value[ $this->field['mode'] . '-bottom' ] : $this->value['bottom'],
                'left'   => isset( $this->value[ $this->field['mode'] . '-left' ] ) ? $this->value[ $this->field['mode'] . '-left' ] : $this->value['left']
            );
        ...
        function enqueue() {
            ...
            wp_enqueue_script(
                ...
                trailingslashit( plugin_dir_url( __FILE__ ) ).'field_spacing' . Redux_Functions::isMin() . '.js',
            ...
            if ($this->parent->args['dev_mode']) {
                wp_enqueue_style(
                    ...
                    trailingslashit( plugin_dir_url( __FILE__ ) ).'field_spacing.css',
                    array(),
                    '1.0',
                    'all'
                );
        ...
        public function output() {
            ...
            if(isset( $this->value['units'] ) && !empty($this->value['units'])){
            ...
            if ( ! empty( $mode ) ) {
                foreach ( $this->value as $key => $value ) {
                    ...
                    if ( isset( $value ) && is_numeric( $num_no_alpha ) ) {
                        $value = (preg_match( '/(px|em|\%|pt|cm|rem)$/', $value ) ? $value : $value . 'px' );
                        $style .= $key . ':' . $value . ';';
                    }
